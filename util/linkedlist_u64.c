...
static node_u64_t *node_u64_init(uint64_t);

#define LLU64_RETURN_FAILURE -1

/**
 * Initialize a linkedlist for uint64_t's. Only fails if malloc fails.
 */
linkedlist_u64_t *ll_u64_init()
{
	linkedlist_u64_t *ll;

	ll = malloc(sizeof(linkedlist_u64_t));
	#ifdef LL_CHECK_MALLOC_FAIL
	if (ll == NULL)
	{
		perror("LL_U64_INIT: Malloc failed");
		return NULL;
	}
	#endif
 ...
 */
int ll_u64_setup(linkedlist_u64_t *list)
{
	if (list == NULL)
	{
		return 0;
 ...
 */
int ll_u64_destroy(linkedlist_u64_t *list)
{
	if (!list)
	{
		return -1;
...
int ll_u64_clear(linkedlist_u64_t *list)
{
	node_u64_t *nextnode;
	if (!list)
	{
 ...
 */
int ll_u64_add(linkedlist_u64_t *list, uint64_t data)
{
	node_u64_t *newnode;
 ...
 */
int ll_u64_add_index(linkedlist_u64_t *list, unsigned long index, uint64_t data)
{
	node_u64_t *newnode;
 ...
 * Runs in O(n) time.
 */
int ll_u64_remove(linkedlist_u64_t *list, uint64_t data)
{
	node_u64_t *prev;
	node_u64_t *next;
 ...
 */
uint64_t ll_u64_remove_index(linkedlist_u64_t *list, unsigned long index)
{
	node_u64_t *previousnode;
	node_u64_t *nextnode;
	uint64_t data;

	if (!list || index >= list->count)
	{
		return LLU64_RETURN_FAILURE;
	}
 ...
 */
uint64_t ll_u64_get(linkedlist_u64_t *list, unsigned long index)
{	
	unsigned long i;
	node_u64_t *currentnode;
 ...
 * list, or -1 if not present. Basically the inverse of ll_u64_get. Runs in O(n) 
 * time, depending where the data is.
 */
unsigned long ll_u64_search(linkedlist_u64_t *list, uint64_t data)
{
	node_u64_t *currentnode;
	unsigned long index;
 ...
 */
int ll_u64_operate(linkedlist_u64_t *list, u64_operate_fn data_function)
{
	node_u64_t *currentnode;
 ...
 */
void ll_u64_append(linkedlist_u64_t *list, linkedlist_u64_t *second)
{
 ...
 * Initializes a node.
 */
static node_u64_t *node_u64_init(uint64_t data)
{
	node_u64_t *n = malloc(sizeof(node_u64_t));
	#ifdef LL_CHECK_MALLOC_FAIL
	if (n == NULL)
	{
