#include <stdlib.h>
#include "hashmap_u64_int.h"

typedef struct hashmap_u64_int_entry_t {
	uint64_t key;
	int value;
} hashmap_u64_int_entry_t;

static hashmap_u64_int_entry_t *hm_u64_int_entry_init(uint64_t key, int value)
{
	hashmap_u64_int_entry_t *entry = malloc(sizeof(hashmap_u64_int_entry_t));
	entry->key = key;
	entry->value = value;
	return entry;
}

static int hm_u64_int_entry_compare(void *a, void *b)
{
	hashmap_u64_int_entry_t *foo = (hashmap_u64_int_entry_t *)a;
	hashmap_u64_int_entry_t *bar = (hashmap_u64_int_entry_t *)b;
	return (bar->key) - (foo->key);
}

static void hm_u64_int_entry_destroy(void *entry)
{
	free(entry);
}
 ...
 */
hashmap_u64_int_t *hm_u64_int_init(unsigned long size)
{
	hashmap_u64_int_t *hm = malloc(sizeof(hashmap_u64_int_t));
	hm_u64_int_setup(hm, size);
	return hm;
}
 ...
 */
int hm_u64_int_setup(hashmap_u64_int_t *hm, unsigned long size)
{
	unsigned long index;
	if (hm == NULL)
	{
	...
	for (index = 0; index < size; index++)
	{
		ll_setup(&(hm->array[index]),
		         hm_u64_int_entry_compare, hm_u64_int_entry_destroy);
	}
 ...
 */
int hm_u64_int_destroy(hashmap_u64_int_t *hm)
{
	if (hm == NULL)
	{
		return -1;
	}
	hm_u64_int_teardown(hm);
	free(hm);
	
	return 0;
}

int hm_u64_int_teardown(hashmap_u64_int_t *hm)
{
	if (hm == NULL)
	{
		return -1;
...
int hm_u64_int_clear(hashmap_u64_int_t *hm)
{
	unsigned long index;
 ...
 */
int hm_u64_int_add(hashmap_u64_int_t *hm, uint64_t key, int value)
{
	hashmap_u64_int_entry_t *entry;
	int retval = 0;
	if (hm == NULL)
	{
		return -1;
	...
	if (hm_u64_int_containskey(hm, key))
	{
		hm_u64_int_removekey(hm, key);
		retval = 1;
	}
	hm->count++; /* works in either case - hm_remove decrements count */
	entry = hm_u64_int_entry_init(key, value);
 ...
 */
int hm_u64_int_containskey(hashmap_u64_int_t *hm, uint64_t key)
{
	hashmap_u64_int_entry_t dummy; /* on the stack not heap for speed */
	if (hm == NULL)
	{
		return -1;
	}
 ...
 */
int hm_u64_int_get(hashmap_u64_int_t *hm, uint64_t key)
{
	node_t *currentnode;
	linkedlist_t *bucket;
	...
	while (currentnode)
	{
		if (key == ((hashmap_u64_int_entry_t *)(currentnode->data))->key)
		{
			return ((hashmap_u64_int_entry_t *)(currentnode->data))->value;
		}
 ...
 */
int hm_u64_int_removekey(hashmap_u64_int_t *hm, uint64_t key)
{
	/* same principle as contains - the ll needs to search for the pair */
	hashmap_u64_int_entry_t dummy;
	int retval;
	if (hm == NULL)
	{
		return -1;
 ...
 * like it, don't use hm_u64_int_operate */
static u64_int_operate_fn pair_operate;
/**
 * For passing to the linkedlist. This function should never be passed to
 * ll_operate unless by the publicly accessible hm_u64_int_operate, or else
 * the data pointer may be invalid.
 */
static void hm_u64_int_entry_operate(void *ptr)
{
	hashmap_u64_int_entry_t *entry = (hashmap_u64_int_entry_t *)ptr;
	pair_operate(entry->key, entry->value);
}
/**
 * Given a function pointer to a u64_int_operate_fn, perform it on every
 * (key,value) pairing in the map.
 */
int hm_u64_int_operate(hashmap_u64_int_t *hm, u64_int_operate_fn pair_fn)
{
	unsigned long i;
	...
	for (i = 0; i < hm->size; i++)
	{
		ll_operate(&(hm->array[i]), hm_u64_int_entry_operate);
	}
